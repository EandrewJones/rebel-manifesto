...
# TODO: Separate into src code and script
class BatchDocumentTranslator(object):
    ...
    def __init__(self, save_dir, job_file, batch):
        ...
        self.is_translated = np.array([job['is_translated']
    ...
    def _get_batch_idx(self):
        """
        Creates a batch of jobs as close to
        specified batchsize as possible
        """

        # Get parameters from self
        batch_size = self.batch
        untranslated_jobs = self.get_untranslated_jobs() # TODO: check run & remove
    ...
    def translate_documents(self):
        ...
        for idx, job in enumerate(self.jobs):
            if idx in np.where(batch_idx)[0]:
                try:
                    job['translation'] = [self.translator.translate(p)
                                          for p in job['paragraphs']]
...
def write_file(job: list, use_translation: bool, base_dir: str):
    ...
    file_name = job['title'] + '_' + job['doc_type'] + \
        '_' + job['date'] + '.txt'
    if len(file_name) > 255:  # clip title length
        file_name = job['title'][0:201] + '_' + \
            job['doc_type'] + '_' + job['date'] + '.txt'
