...
from src.scrapers import SudanJEMScraper
import json
import logging
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename='logs/scrape_jem.log',
        filemode='w',
        format='%(name)s - %(levelname)s - %(message)s'
    )

    # Instantiate scraper
    with open('webscraping/params/jem_params.json') as infile:
        params = json.load(infile)

    scraper = SudanJEMScraper(**params)
