...
from src.scrapers import IsraelPFLPScraper
import json
import logging
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename='logs/scrape_pflp.log',
        filemode='w',
        format='%(name)s - %(levelname)s - %(message)s'
    )

    # Instantiate scraper
    with open('webscraping/params/pflp_params.json') as infile:
        params = json.load(infile)

    scraper = IsraelPFLPScraper(**params)
